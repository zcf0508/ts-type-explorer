...
import { isEmpty, isNonEmpty, wrapSafe } from './objectUtil';
...
function getChildren(
  ...
  const localizeOpts = (info: TypeInfo, opts?: LocalizeOpts): { info: TypeInfo, opts?: LocalizeOpts } => ({
    info,
    opts,
  });
  const localize = (info: TypeInfo): { info: TypeInfo } => localizeOpts(info);
  const createChild = (localizedInfo: LocalizedTypeInfo): { localizedInfo: LocalizedTypeInfo } => ({
    localizedInfo,
  });
    ...
    if (typeParameters && typeArguments) {
      ...
      return [
        ...(typeParameters
          ...
          : []),
        ...(typeArguments
          ? [getTypeArgumentList(typeArguments)]
          : []),
      ];
    }
  }
  ...
  function getLocalizedSignatureChildren(
    ...
    info: TypeInfo
    opts?: LocalizeOpts | undefined
  } | TypeInfoChildren[number]> {
    return [
...
function getKind(info: ResolvedTypeInfo): string {
  const kindText = (kind: LocalizableKind, ...args: string[]): string =>
    getKindText(
      kind,
      {

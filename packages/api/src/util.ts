...
import { filterUndefined, isEmpty, wrapSafe } from './objectUtil';
import {
  CheckFlags,
...
export function getNodeType(ctx: TypescriptContext, node: ts.Node): ts.Type | undefined {
  ...
  if (symbolType && isValidType(symbolType)) {
    return symbolType;
  }
  ...
  if (
    ...
    if (expressionType && isValidType(expressionType)) {
 ...
 */
export function checkExpression(ctx: TypescriptContext, node: ts.Node): ts.Type | undefined {
 ...
 * @internal
 */
export function createType(ctx: TypescriptContext, flags: ts.TypeFlags): ts.Type {
...
export function isReadonlyTupleType(ctx: TypescriptContext, type: ts.Type): boolean {
  return !!(
...
export function getTypeParameters(
  ...
  if (isClassOrInterfaceType(ctx, type)) {
    ...
    if (isEmpty(typeParameterDeclarations)) {
      return undefined;
    }
...
export function pseudoBigIntToString(value: ts.PseudoBigInt): string {
...
function getLeftHandSideExpression(node: ts.CallLikeExpression): ts.Expression | ts.JsxNamespacedName {
  if ('tag' in node) {
    return node.tag;
  }
...
export function getCallLikeExpression(ctx: TypescriptContext, node: ts.Node): ts.CallLikeExpression | undefined {
  const { ts } = ctx;
 ...
 * @param node
 */
export function narrowDeclarationForLocation(node: ts.Declaration): ts.Declaration {

...
import type { APIConfig, RecursiveExpandContext, TypescriptContext } from './types';
import type {
  ObjectTypeInternal,
  SymbolInternal,
  SymbolName,
...
function _recursivelyExpandType(
  ...
  function expandType(): ts.Type {
    ...
    if (processedTypes.length === 1) {
      if (types.length === 1) {
        seen.set(types[0], processedTypes[0]);
      }
  ...
  function createAnonymousObjectType(): ObjectTypeInternal {
  ...
  function removeTypeAlias(type: ts.Type): ts.Type {

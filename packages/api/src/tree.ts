...
interface GenerateTypeTreeArgs {
  symbolOrType: SymbolOrType
...
function _generateTypeTree(
  ...
  const createTypeInfo = (args: Omit<GenerateTypeTreeArgs, 'typeInfo'>): TypeInfo => {
  ...
  function _typeTree(
    ...
    function getSymbolLocations(symbol?: ts.Symbol): SourceFileLocation[] | undefined {
      return wrapSafe(filterUndefined)(
        symbol?.getDeclarations()?.map(getDeclarationLocation),
      );
    }

    function getRecoverableSymbolLocations(symbol?: ts.Symbol): SourceFileLocation[] | undefined {
    ...
    function getDeclarationLocation(declaration: ts.Declaration): SourceFileLocation | undefined {
...
function resolveSignature(
  ...
  node?: ts.Node,
    ...
    isConstructCallExpression: boolean
    signature?: SignatureInternal
    signatureTypeArguments?: readonly ts.Type[]
    signatureTypeParameters?: readonly ts.TypeParameter[]
...
export function getTypeInfoChildren(info: TypeInfo): TypeInfo[] {
  const mapSignatureInfo = (signature: SignatureInfo): (TypeInfo | undefined)[] => [
...
export function getTypeInfoSymbols(info: TypeInfo): SymbolInfo[] {
  ...
  function _getTypeInfoSymbols(info: TypeInfo): (SymbolInfo | undefined)[] {
    ...
    function mapSignature(signature: SignatureInfo): (SymbolInfo | undefined)[] {
...
class TypeInfoFactory {
  ...
  assignTypeInfo(target: TypeInfo, source: TypeInfo): void {
    Object.assign(target, source);
    this.unassigned--;
  }

  assertNoUnassigned(): void {
